...
unsigned char code wei_code_ca[]={~0x20,~0x10,~0x08,~0x04,~0x02,~0x01};//六位共阴数码管位码表，分别控制自左及右的一位数码管亮，其余全灭
...
{
	dat+=1;
	disp_buf_change(dat>=100000?dat%1000000/100000:10,dat>=10000?dat%100000/10000:10,dat>=1000?dat%10000/1000:10,dat>=100?dat%1000/100:10,dat>=10?dat%100/10:10,dat%10);//
}
//
void main()
{
	...
	int0_init();//外部中断0初始化
	EA=1;//总的中断允许
	disp_buf_change(dat>=100000?dat%1000000/100000:10,dat>=10000?dat%100000/10000:10,dat>=1000?dat%10000/1000:10,dat>=100?dat%1000/100:10,dat>=10?dat%100/10:10,dat%10);//初始显示0
	while(1);
}